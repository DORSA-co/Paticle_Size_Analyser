...
# @cython.boundscheck(False) # turn off bounds-checking for entire function
# @cython.wraparound(False)  # turn off negative index wrapping for entire function
# def sieve2(numpy.ndarray[DTYPE_float64, ndim=1] xs,
#         numpy.ndarray [DTYPE_float64, ndim=2]ranges,
#         numpy.ndarray[DTYPE_float64, ndim=1] weights):
#     #returns idx range of each x

#     cdef int i,j
#     cdef int xs_count = xs.shape[0]
#     cdef int ranges_count = ranges.shape[0]
#     cdef numpy.ndarray[DTYPE_int32, ndim=1] idxs = numpy.ones((xs_count,), dtype = numpy.int32) * -1
#     cdef numpy.ndarray[DTYPE_float64, ndim=1] hist = numpy.zeros((ranges_count,), dtype = numpy.float64)
...
@cython.boundscheck(False) # turn off bounds-checking for entire function
@cython.wraparound(False)  # turn off negative index wrapping for entire function
def sieve_particle(numpy.ndarray[DTYPE_float64, ndim=1] x,
        numpy.ndarray [DTYPE_float64, ndim=2]ranges):
    #returns idx range of each x
